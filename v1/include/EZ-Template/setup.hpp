...
const int  L_CHASSIS_PORTS[MOTORS_PER_SIDE] = {-7, -8,  9}; // Ports, the first number will be used for sensing
const int  R_CHASSIS_PORTS[MOTORS_PER_SIDE] = { 4,  3, -2}; // Ports, the first number will be used for sensing
...
const float RATIO     = 1.875; // External drive ratio (MUST BE DECIMAL)
...
///
const float ACTIVE_BRAKE_KP = 0.1; // Constant for activebrake (increase this to make it more aggressive, 0.1 is recommended)
